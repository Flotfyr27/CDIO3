    ...
    public GuiHandler(Field[] fields){
        for(int i = 0; i < gui_field.length; i++){
            if(fields[i].getClass().equals(EmptyField.class) && i == 0){
                gui_field[i] = (new GUI_Start(fields[i].getName(), "", fields[i].getSubtext(), Color.lightGray, null));
            }else if(fields[i].getClass().equals(PropertyField.class)){
                PropertyField propertyField = (PropertyField) fields[i];
                gui_field[i] = (new GUI_Street(fields[i].getName(), "", fields[i].getSubtext(), Integer.toString(propertyField.getRent()), Color.lightGray, null));
            }else if(fields[i].getClass().equals(EmptyField.class)){
                gui_field[i] = (new GUI_Empty(Color.lightGray, null, fields[i].getName(), "", fields[i].getSubtext()));
            }else if(fields[i].getClass().equals(ChanceField.class)){
                gui_field[i] = (new GUI_Chance(fields[i].getName(), "", fields[i].getSubtext(), Color.lightGray, null));
            }
        }
